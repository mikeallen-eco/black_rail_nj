...
Note: this was run on the Amarel HPC cluster. See R script, BASH script, and BUGS model with the name 'ensp_ebird_cell_predict_yrcon_for_Amarel' in the "scripts/amarel/" folder.

# Create tables of model outputs (year factor and continuous)
```{r}

 ee <-
  readRDS("D:/black_rail/big_output_2021_10/rda/JAGS_ensp_ebird_p.ord.dur_psi.yr.ss.hm.tb.ld.im_12000iter_pred_full.Rdata")
...
# below, I export scripts to run on the Amarel HPC in parallel
ensp_ebird_cell_pred_dur_med_yrcont = lapply(1:10, #nrow(pp), 
       FUN = function(x){
         print(x)
...
write.csv(ensp_ebird_cell_pred_dur_med_yrcont, 
 "ensp_ebird_cell_pred_dur_med_yrcont.csv",
          row.names = F)    

# create a script to do this on the Amarel HPC
cat('
# for Amarel HPC
...
ensp_ebird_cell_pred_dur_med_yrcont = lapply(1:nrow(pp), 
       FUN = function(x){
         median(plogis(sapply(1:12000, function(y){
           beta0[y] +
           (beta[y,1] * (8-4.889157)) +
...
write.csv(ensp_ebird_cell_pred_dur_med_yrcont, 
 "/scratch/mcallen/blra_pred_ycont/ensp_ebird_cell_pred_dur_med_yrcont.csv",
    ...
    ',
file = "scripts/amarel/ensp_ebird_cell_pred_dur_med_yrcont_Amarel.R")


# create a BASH script to do this on the Amarel HPC
f <- file("scripts/amarel/ensp_ebird_cell_pred_dur_med_yrcont_Amarel.sh", open="wb") # this part prevents "DOS encoding"
...
# placeholder to stitch together into a map after HPC runs  
ensp_cell_pred_med <-
  read.csv("D:/black_rail/big_output_2021_10/csv/ensp_cell_predict_out_summary.csv") %>%
  bind_cols(b.brick_pred_pts) %>%
  dplyr::select(x, y, med) %>% 
...
```
# Output sh script to run JAGS model (continuous year) and predict to grid cells using Amarel HPC cluster
```{r}
f <- file("scripts/amarel/ensp_ebird_cell_predict_yrcon_for_Amarel.sh", open="wb") # this part prevents "DOS encoding"
...
# This line tells Amarel you are running an R script, and it links this
# job file to the script that you want to run. Make sure that this file path matches the path to the
# script in your home directory. 
