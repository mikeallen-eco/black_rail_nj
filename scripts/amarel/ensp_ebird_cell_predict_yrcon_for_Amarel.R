...
b.site.cov.ensp_ebird <- read.csv("/scratch/mcallen/blra_pred/b.site.cov.ensp_ebird.csv") %>%
  ...
  mutate(yearfac = as.factor(yearfac),
         year = as.numeric(yearfac) - mean(as.numeric(yearfac))) %>%
  dplyr::select(-yearfac)
...
occDM <- model.matrix(~ year + ss.500 + sharp_hm.500 + sharp_tb.500 + ldev.100 + imp.500, data = b.site.cov.ensp_ebird)[,-1] # Drop first col.
...
# run model
ensp_ebird_p.ord_psi.yrcon.ss.hm.tb.ld.im <-
  jagsUI::jags(
    ensp_ebird.jags.data,
    inits,
    params,
    "/home/mcallen/blra_pred/ensp_ebird_cell_predict_yrcon_for_Amarel.txt",
    n.chains = nc,
    n.thin = nt,
    n.iter = ni,
    n.burnin = nb,
...
rhat <- do.call(c, ensp_ebird_p.ord_psi.yrcon.ss.hm.tb.ld.im$Rhat)
print(rhat)

posteriors = data.frame(
  q2.5 =ensp_ebird_p.ord_psi.yrcon.ss.hm.tb.ld.im$q2.5$psi.marsh.cells,
  q50 =ensp_ebird_p.ord_psi.yrcon.ss.hm.tb.ld.im$q50$psi.marsh.cells,
  q97.5 = ensp_ebird_p.ord_psi.yrcon.ss.hm.tb.ld.im$q97.5$psi.marsh.cells)
  
write.csv(posteriors, 
          "/scratch/mcallen/blra_pred/ensp_ebird_cell_predict_out_yrcon.csv",
          row.names = F)